...
class SCFP_Form extends Agp_Module {
    ...
    private function saveForm() {
        ...
        if (!is_wp_error($this->postId )) {
            ...
            foreach ($this->data as $key => $value) {
                if ($result) {
                    $meta_key = 'scfp_' . $key;
                    $meta_value = apply_filters( 'scfp_form_save_field', apply_filters( 'scfp_form_save_field_' . $key, nl2br($value), $value, $this), $key, $value, $this);
    ...
    public function notification() {
        ...
        if (empty($settings['disable'])) {
            ...
            $subject = apply_filters( 'scfp_form_admin_notification_subject', stripslashes ( !empty($settings['subject']) ? $settings['subject'] : '' ), $this);
            $message = apply_filters( 'scfp_form_admin_notification_message', stripslashes ( $this->applyEmailVars( nl2br(!empty($settings['message']) ? $settings['message'] : '') ) ), $this);
        ...
        if (empty($settings['user_disable'])) {
            ...
            $subject = apply_filters( 'scfp_form_user_notification_subject', stripslashes ( !empty($settings['user_subject']) ? $settings['user_subject'] : '' ), $this);
            $message = apply_filters( 'scfp_form_user_notification_message', stripslashes ( $this->applyEmailVars( nl2br(!empty($settings['user_message']) ? $settings['user_message'] : '') ) ), $this);
